...
class Board:
    ...
    def __init__(self, text):
        ...
        self.width = max(
            list(map(lambda x: len(self._lines[x]), range(len(self._lines))))
        )
    ...
    def get_moves(self):
        ...
        for direction in self._directions:
            ...
            beyond_pos_x, beyond_pos_y = self._get_beyond_coords(
    ...
    def execute_move(self, move):
        ...
        if (
            target_character != self._characters["$"]
            and target_character != self._characters["*"]
        ):
            # The new location
            if target_character == self._characters["."]:
    ...
    def _find_player(self):
        ...
        for x in range(self.width):
            for y in range(self.height):
                if (
                    self.board[x][y] == self._characters["@"]
                    or self.board[x][y] == self._characters["+"]
                ):
                    return (x, y)
        return None
